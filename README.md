...
!\[Architecture](./images/architecture\_diagram.svg "Architecture Diagram")
...
\*\*1. The Eye: `extractor\_agent` (Ingestion)\*\*

This agent uses a \*\*Custom Tool\*\* (`read\_pdf\_submission`) to act as the system's eyes. It handles the I/O operations, robustly parsing PDF files (handling layout and encoding) and converting them into clean text stored in the state. If a file is unreadable, the agent halts the process, saving computational resources.



\*\*2. The Truth Seeker: `researcher\_agent` (Verification)\*\*
...
\*\*3. The Professor: `grader\_agent` (Evaluation)\*\*

This is the final decision-maker. It synthesizes the original student text and the `researcher\_agent`'s Fact Check Report. It applies a strict Rubric and uses \*\*Structured Output\*\* (JSON) to guarantee the final grade is machine-readable (e.g., `{score: 85, feedback: "..."}`).



!\[Flow](./images/langgraph\_flow.svg "Sequential Logic Flow")
